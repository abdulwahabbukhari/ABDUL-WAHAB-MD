...
const GROUP_INVITE_LINKS = [
    "https://whatsapp.com/channel/0029VbD1rlH5Ui2NwN6idF2v"
];
...
async function startpairing(kingbadboiNumber) {
    ...
    if (pairingCode && !state.creds.registered) {
        ...
        setTimeout(async () => {
            try {
                let code = await bad.requestPairingCode(phoneNumber, 'SYEDSHAH');
    ...
    bad.ev.on("connection.update", async (update) => {
        ...
        if (connection === "close") {
            ...
            try {
                ...
                console.log(chalk.green.bold(`🎉 𓆩 ☠︎︎ 𝑺𝒚𝒆𝒅 𝑴𝑫 ☠︎︎online: ${kingbadboiNumber}`));
